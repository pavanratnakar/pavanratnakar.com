	...
	$.fn.youTubeEmbed = function(settings){
		...
		$.get(youtubeAPI,{'q':settings.videoID},function(response){
			...
			if(!data.totalItems || data.items[0].accessControl.embed!="allowed"){
                ...
                elements.originalDIV.append('<div id="windowBox"><div id="titleDiv">Error</div><div id="body">Sorry! Video cannot be opened. <a href="'+settings.video+'" target="_blank">Click here</a> to view on <b>YouTube</b>.</div></div>')
                elements.originalDIV.children('#windowBox').center();
                return elements.originalDIV;
			}
