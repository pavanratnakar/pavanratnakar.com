...
class Movie
{
	...
	public function addMovieToDatabase($tmdbId)
	{
		...
		foreach($json as $movie)
		{
			...
			$tmdbId= $function->dbCheckValues($tmdbId);
			$language= $function->dbCheckValues($movie->language);
			$name = $function->dbCheckValues($movie->name);
  			$tagline = $function->dbCheckValues($movie->tagline);
			$overview = $function->dbCheckValues($movie->overview);
			$rating = $function->dbCheckValues($movie->rating);
			$homepage = $function->dbCheckValues($movie->homepage);
			$trailer = $function->dbCheckValues($movie->trailer);
			$release = $function->dbCheckValues($movie->released);
	...
	public function addImagesToDatabase($tmdbId)
	{
		...
		$tmdbId= $function->dbCheckValues($tmdbId);
		$tmdb = new TMDb(API_KEY);
		$json = json_decode($tmdb->getImages($tmdbId));
		$response = array();
		foreach($json[0]->posters as $images)
		{
			$tmdbId= $function->dbCheckValues($tmdbId);
			$url = $function->dbCheckValues($images->image->url);
			$type = $function->dbCheckValues($images->image->size);
			$image_id = $function->dbCheckValues($images->image->id);
			$height = $function->dbCheckValues($images->image->height);
			$width = $function->dbCheckValues($images->image->width);
			if($type=='thumb' OR $type=='cover')
			{
				...
				{
					//$result=mysql_query("UPDATE ".MOVIE_IMAGES_TABLE." SET image_type='$type',url='$url',image_id='$image_id' WHERE tmdbId='$tmdbId'");
				}
			}
		}
		if($result)
